...
export default function AgentCreate() {
  ...
  const { configs, fetchConfigs } = useModelConfigStore();
        ...
        <form onSubmit={handleSubmit} className="bg-white border border-gray-200 rounded-xl p-6 space-y-5">
          ...
          {configs.length > 0 && (
            <div className="pb-4 border-b border-gray-100">
              ...
              <p className="text-xs text-gray-400 mb-2">选择后自动填入 API 地址、Key 和模型名称，仍可手动修改</p>
              <select
                onChange={e => handleSelectConfig(e.target.value)}
                defaultValue=""
                className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-colors bg-white"
              >
                <option value="">-- 选择模型配置 --</option>
                {configs.map(c => (
                  <option key={c.id} value={c.id}>{c.name} ({c.llm_model})</option>
                ))}
              </select>
            </div>
          )}
